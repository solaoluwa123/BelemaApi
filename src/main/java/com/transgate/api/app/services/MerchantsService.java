...
public class MerchantsService implements MerchantsInterface {
    ...
    private Logger logger = Logger.getLogger(MerchantsService.class.getName());
    ...
    public ResponseEntity GetByPTSP(String ptsp) {
        logger.info(String.format("Entering GetByPTSP with ptsp: %s", ptsp));
        NetworkResponse networkResponse = new NetworkResponse();
        try {
            String SQL;
            List<CardsMerchantModel> merchants;

            SQL = "SELECT a.id, a.merchant_id, a.merchant_name, a.merchant_state, a.merchant_country, "
                    + "a.merchant_category_code, a.merchant_service_charge, a.msc_cap, a.date_created, "
                    + "a.delete_flag, a.edit_flag, a.create_flag "
                    + "FROM sparkpay.merchants a "
                    + "LEFT JOIN sparkpayweb_db.tbl_map_merchants_ptsps b "
                    + "ON a.merchant_id = b.merchant_id "
                    + "WHERE b.ptsp_id = ?";
            logger.info(String.format("Executing SQL for merchants by PTSP: %s", SQL));
            merchants = jdbcTemplate.query(SQL, new Object[]{ptsp}, new MerchantsMapper());
            logger.info(String.format("Retrieved %d merchants for PTSP %s", merchants.size(), ptsp));

            SQL = "SELECT MIN(date_created) FROM sparkpay.merchants";
            logger.info(String.format("Executing SQL for min date: %s", SQL));
            String minDate = jdbcTemplate.queryForObject(SQL, String.class);
            logger.info(String.format("Min date_created: %s", minDate));
...
//            networkResponse.setMeta(meta);
            ...
            logger.info(String.format("Error in GetByPTSP: %s", ex.getMessage()));
            return responseManager.ResponseInternalServerError();
        }
    }
