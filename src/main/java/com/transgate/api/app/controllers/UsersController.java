...
public class UsersController {
    ...
    public ResponseEntity SignUser(@RequestHeader(value = "auth") String header,
        ...
        logger.info("/user/generate-token called");
        logger.info(String.format("SignUser called with auth header='%s', username='%s'", header, login.getUsername()));
        NetworkResponse response = new NetworkResponse();
        // check static header
        if (validators.validHeaderExternal().equals(header)) {
            logger.info("Header matched validHeaderExternal()");
            String token = validators.GenerateJSONWebToken(login.getUsername());
            logger.info(String.format("Generated JWT for user '%s': %s", login.getUsername(), token));
            boolean validUser = usersInterface.LoginExternal2(login.getUsername(), token);
            if (validUser) {
                response.setMessage(token);
                response.setCode(200);
                response.setStatus("success");
                logger.info("Returning 200 Success with token");
                return responseManager.ResponseOk(response);
            }
        } else {
            logger.info("Header did not match validHeaderExternal(), attempting LoginExternal()");
            String bearerToken = header.contains(" ") ? header.split(" ")[1] : "";
            logger.info(String.format("Extracted bearer token='%s' for user '%s'", bearerToken, login.getUsername()));
            boolean validUser = usersInterface.LoginExternal(login.getUsername(), bearerToken);
            logger.info(String.format("LoginExternal() returned %s for user '%s'", validUser, login.getUsername()));
            if (validUser) {
                logger.info("LoginExternal() succeeded, generating JWT");
                String token = validators.GenerateJSONWebToken(login.getUsername());
                logger.info(String.format("Generated JWT for user '%s': %s", login.getUsername(), token));
                response.setMessage(token);
                response.setCode(200);
            ...
            logger.info(String.format("LoginExternal() failed for user '%s'", login.getUsername()));
        }
        logger.info("Authorization failed, returning InvalidAuthorizationHeader");
